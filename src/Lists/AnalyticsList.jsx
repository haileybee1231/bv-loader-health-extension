...
class AnalyticsList extends React.Component {
  ...
  getCookie = cookie =>
    _get(this.props.BVA, `trackers[bv-loader]._id.${cookie}`);
  ...
  render() {
    ...
    return (
      <React.Fragment>
        ...
        >
          <i
            className={analyticsOpen ? 'icon-chevron-up' : 'icon-chevron-down'}
          />
            ...
            <table style={{ width: '80%', margin: 'auto' }}>
              <tbody>
                <TableRow
                  ...
                  value={String(this.getCookie('BVID'))}
                />
                <TableRow
                  name={'Third Party Cookie'}
                  value={String(this.getCookie('BVSID'))}
                />
                <TableRow
                  name={'Anonymous'}
                  /* eslint-disable-next-line no-extra-boolean-cast */
                  value={!!anonymous ? 'true' : <em>false</em>}
                />
              </tbody>
            </table>
            ...
            <div style={{ width: '100%', margin: 'auto' }}>
              {analyticsArr.length ? (
                analyticsArr.map((analyticEventTuple, index) => {
                  const value =
                    analyticEventTuple[1].__wrapped__ || analyticEventTuple[1];
