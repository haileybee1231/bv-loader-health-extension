...
const FlexDetails = ({
  ...
  <React.Fragment>
    ...
    {flexDetails && flexDetails.layouts && (
      Object.keys(flexDetails.layouts).map((layout, index) => {    
        const containerNodeList = document.querySelectorAll(`[data-bv-show="${layout}"]`);
        const containerObj = [...containerNodeList]
          .map(container => container.dataset)[0];
        ...
        return (
          <React.Fragment key={index}>
            ...
            <table>
              <tbody>
                {Object.entries(flexDetails.layouts[layout]).map((tuple, index) =>
                  <TableRow
                    name={tuple[0]}
                    value={
                      tuple[0] === '_analytics'
